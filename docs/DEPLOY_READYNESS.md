...
| CI build Docker | LISTO | Jobs `docker-api` y `docker-frontend` fallan el pipeline si el Dockerfile está roto |
...
| Docker build en CI | LISTO | Build de `docker-api` y `docker-frontend` bloquea el merge si falla |
...
- CI falla si el build Docker se rompe (jobs `docker-api` / `docker-frontend` sin `continue-on-error`).
